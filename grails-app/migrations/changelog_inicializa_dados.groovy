	...
	changeSet(author: "eduardo", id: "INIT_SEQUENCES") {
		grailsChange {
			change {
				sql.execute("""
					SELECT
						setval('seq_agenda', COALESCE( SELECT MAX( id ) FROM agenda))
						, setval('seq_anuncio', COALESCE( SELECT MAX( id ) FROM anuncio))
						, setval('seq_aplicacao', COALESCE( SELECT MAX( id ) FROM aplicacao))
						, setval('seq_bairro', COALESCE( SELECT MAX( id ) FROM bairro))
						, setval('seq_bairro_corret', COALESCE( SELECT MAX( id ) FROM bairro_corretagem))
						, setval('seq_campanha', COALESCE( SELECT MAX( id ) FROM campanha))
						, setval('seq_carteira_imovel', COALESCE( SELECT MAX( id ) FROM carteira_imovel))
						, setval('seq_cidade', COALESCE( SELECT MAX( id ) FROM cidade))
						, setval('seq_cidade_corret', COALESCE( SELECT MAX( id ) FROM cidade_corretagem))
						, setval('seq_classe_imovel', COALESCE( SELECT MAX( id ) FROM classe_imovel))
						, setval('seq_classe_interessado', COALESCE( SELECT MAX( id ) FROM classe_interessado))
						, setval('seq_concorrente', COALESCE( SELECT MAX( id ) FROM concorrente))
						, setval('seq_condominio', COALESCE( SELECT MAX( id ) FROM condominio))
						, setval('seq_corretor', COALESCE( SELECT MAX( id ) FROM corretor))
						, setval('seq_edificio', COALESCE( SELECT MAX( id ) FROM edificio))
						, setval('seq_empresa', COALESCE( SELECT MAX( id ) FROM empresa))
						, setval('seq_equipe', COALESCE( SELECT MAX( id ) FROM equipe))
						, setval('seq_estado', COALESCE( SELECT MAX( id ) FROM estado))
						, setval('seq_evento', COALESCE( SELECT MAX( id ) FROM evento))
						, setval('seq_finalidade_imovel', COALESCE( SELECT MAX( id ) FROM finalidade_imovel))
						, setval('seq_grupo', COALESCE( SELECT MAX( id ) FROM grupo))
						, setval('seq_imovel', COALESCE( SELECT MAX( id ) FROM imovel))
						, setval('seq_interessado', COALESCE( SELECT MAX( id ) FROM interessado))
						, setval('seq_logradouro', COALESCE( SELECT MAX( id ) FROM logradouro))
						, setval('seq_loja', COALESCE( SELECT MAX( id ) FROM loja))
						, setval('seq_menu', COALESCE( SELECT MAX( id ) FROM menu))
						, setval('seq_midia_tipo', COALESCE( SELECT MAX( id ) FROM midia_tipo))
						, setval('seq_moeda', COALESCE( SELECT MAX( id ) FROM moeda))
						, setval('seq_pessoa', COALESCE( SELECT MAX( id ) FROM pessoa))
						, setval('seq_proposta', COALESCE( SELECT MAX( id ) FROM proposta))
						, setval('seq_quadro_chaves', COALESCE( SELECT MAX( id ) FROM quadro_chaves))
						, setval('seq_regiao_corret', COALESCE( SELECT MAX( id ) FROM regiao_corretagem))
						, setval('seq_regra', COALESCE( SELECT MAX( id ) FROM regra))
						, setval('seq_request_map', COALESCE( SELECT MAX( id ) FROM requestmap))
						, setval('seq_segmento_imovel', COALESCE( SELECT MAX( id ) FROM segmento_imovel))
						, setval('seq_tipo_dependencia', COALESCE( SELECT MAX( id ) FROM tipo_dependencia))
						, setval('seq_tipo_endereco', COALESCE( SELECT MAX( id ) FROM tipo_endereco))
						, setval('seq_tipo_imovel', COALESCE( SELECT MAX( id ) FROM tipo_imovel))
						, setval('seq_tipo_logradouro', COALESCE( SELECT MAX( id ) FROM tipo_logradouro))
						, setval('seq_usuario', COALESCE( SELECT MAX( id ) FROM usuario))
						, setval('seq_veiculo_comunicacao', COALESCE( SELECT MAX( id ) FROM veiculo_comunicacao))
						, setval('seq_venda', COALESCE( SELECT MAX( id ) FROM venda));
				""")
			}
		}
	}
