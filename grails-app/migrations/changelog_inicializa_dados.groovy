	...
	changeSet(author: "eduardo", id: "INIT_SEQUENCES") {
		grailsChange {
			change {
				sql.execute("""
					SELECT
						setval('seq_agenda', COALESCE( SELECT MAX( id ) FROM agenda, 0))
						, setval('seq_anuncio', COALESCE( SELECT MAX( id ) FROM anuncio, 0))
						, setval('seq_aplicacao', COALESCE( SELECT MAX( id ) FROM aplicacao, 0))
						, setval('seq_bairro', COALESCE( SELECT MAX( id ) FROM bairro, 0))
						, setval('seq_bairro_corret', COALESCE( SELECT MAX( id ) FROM bairro_corretagem, 0))
						, setval('seq_campanha', COALESCE( SELECT MAX( id ) FROM campanha, 0))
						, setval('seq_carteira_imovel', COALESCE( SELECT MAX( id ) FROM carteira_imovel, 0))
						, setval('seq_cidade', COALESCE( SELECT MAX( id ) FROM cidade, 0))
						, setval('seq_cidade_corret', COALESCE( SELECT MAX( id ) FROM cidade_corretagem, 0))
						, setval('seq_classe_imovel', COALESCE( SELECT MAX( id ) FROM classe_imovel, 0))
						, setval('seq_classe_interessado', COALESCE( SELECT MAX( id ) FROM classe_interessado, 0))
						, setval('seq_concorrente', COALESCE( SELECT MAX( id ) FROM concorrente, 0))
						, setval('seq_condominio', COALESCE( SELECT MAX( id ) FROM condominio, 0))
						, setval('seq_corretor', COALESCE( SELECT MAX( id ) FROM corretor, 0))
						, setval('seq_edificio', COALESCE( SELECT MAX( id ) FROM edificio, 0))
						, setval('seq_empresa', COALESCE( SELECT MAX( id ) FROM empresa, 0))
						, setval('seq_equipe', COALESCE( SELECT MAX( id ) FROM equipe, 0))
						, setval('seq_estado', COALESCE( SELECT MAX( id ) FROM estado, 0))
						, setval('seq_evento', COALESCE( SELECT MAX( id ) FROM evento, 0))
						, setval('seq_finalidade_imovel', COALESCE( SELECT MAX( id ) FROM finalidade_imovel, 0))
						, setval('seq_grupo', COALESCE( SELECT MAX( id ) FROM grupo, 0))
						, setval('seq_imovel', COALESCE( SELECT MAX( id ) FROM imovel, 0))
						, setval('seq_interessado', COALESCE( SELECT MAX( id ) FROM interessado, 0))
						, setval('seq_logradouro', COALESCE( SELECT MAX( id ) FROM logradouro, 0))
						, setval('seq_loja', COALESCE( SELECT MAX( id ) FROM loja, 0))
						, setval('seq_menu', COALESCE( SELECT MAX( id ) FROM menu, 0))
						, setval('seq_midia_tipo', COALESCE( SELECT MAX( id ) FROM midia_tipo, 0))
						, setval('seq_moeda', COALESCE( SELECT MAX( id ) FROM moeda, 0))
						, setval('seq_pessoa', COALESCE( SELECT MAX( id ) FROM pessoa, 0))
						, setval('seq_proposta', COALESCE( SELECT MAX( id ) FROM proposta, 0))
						, setval('seq_quadro_chaves', COALESCE( SELECT MAX( id ) FROM quadro_chaves, 0))
						, setval('seq_regiao_corret', COALESCE( SELECT MAX( id ) FROM regiao_corretagem, 0))
						, setval('seq_regra', COALESCE( SELECT MAX( id ) FROM regra, 0))
						, setval('seq_request_map', COALESCE( SELECT MAX( id ) FROM requestmap, 0))
						, setval('seq_segmento_imovel', COALESCE( SELECT MAX( id ) FROM segmento_imovel, 0))
						, setval('seq_tipo_dependencia', COALESCE( SELECT MAX( id ) FROM tipo_dependencia, 0))
						, setval('seq_tipo_endereco', COALESCE( SELECT MAX( id ) FROM tipo_endereco, 0))
						, setval('seq_tipo_imovel', COALESCE( SELECT MAX( id ) FROM tipo_imovel, 0))
						, setval('seq_tipo_logradouro', COALESCE( SELECT MAX( id ) FROM tipo_logradouro, 0))
						, setval('seq_usuario', COALESCE( SELECT MAX( id ) FROM usuario, 0))
						, setval('seq_veiculo_comunicacao', COALESCE( SELECT MAX( id ) FROM veiculo_comunicacao, 0))
						, setval('seq_venda', COALESCE( SELECT MAX( id ) FROM venda, 0));
				""")
			}
		}
	}
